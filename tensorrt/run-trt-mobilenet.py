...
def get_input_data(input_file,batch_size):
    with open(input_file, "r") as file:
        ...
        # 使用正则表达式匹配浮点数模式
    pattern = r'\d+\.\d+e[+-]\d+'  # 匹配浮点数的模式，包括指数形式的数字

        # 使用 findall 查找所有匹配的浮点数
    float_numbers = re.findall(pattern, data_lines)

        # 将指数形式的浮点数转换为标准浮点数
    custom_input_data = [float(number) for number in float_numbers]
...
if __name__ == "__main__":
    ...
    onnx_file_path="/vodla_workload/"+model_name+"/models/mobilenet_v2.onnx"
    input_file="/vodla_workload/"+model_name+"/data/input_data.txt"
