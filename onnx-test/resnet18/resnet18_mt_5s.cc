...
class Worker {
 ...
 public:
  ...
  Worker(odla_device device, odla_executable exec, int batch, std::atomic_int& cnts,
         void* arg0_data, int total_iters, int sleep_ms)
      : device(device), 
        exec(exec),
        cnts(cnts),
  ...
  Worker(odla_device device, odla_resource_location loc, int batch,
         std::atomic_int& cnts, void* arg0_data, int total_iters, int sleep_ms)
      : device(device),
        cnts(cnts),
        batch(batch),
        total_iters(total_iters),
  ...
  void Run() {
    ...
    while ((cnts.fetch_add(1) < total_iters) && !error_flag) {
      ...
      if (worker_id == 0) {
        ...
        timestamp_t nowtime = Now();
        double dur = GetDuration(last_printtime, nowtime);

        if (dur >= 5) {

          last_printtime = nowtime;
          int cnt_5s = cnts - last_cnts;
          last_cnts = cnts;

          // qps latency
          int qps = cnt_5s * batch / dur;
          std::cout << "Average QPS in 5s: " << qps << std::endl;
          std::cout << "Average latency in 5s: " << 1e3 * dur / cnt_5s << "ms"
                    << std::endl;
        }
      }
 ...
 private:
  ...
  // for 5s qps & latency
  inline static std::atomic_int n;
  int worker_id;
  int last_cnts = 0;
  timestamp_t last_printtime = Now();
...
int main(int argc, char** argv) {
  ...
  if (argc < 5) {
    std::cout << "Usage: " << "$exe batch iters thread_num enginefile sleep_ms DYN(opt)" << std::endl;
    std::cout << "Use OEF eg: " << "/tmp/exes/resnet 4 1000 4 /tmp/oefs/resnet.oef 10 DYN " << std::endl;
    std::cout << "Use trt-engine eg: " << "/tmp/exes/resnet 4 10000 4 /host/trt-engines/engines/resnet-dyn-max_b512-fp16-NV_A10.engine 10 DYN" << std::endl;
    return 1;
  }
  ...
  int sleep_ms = atoi(argv[5]);
  bool dyn_batch = argc >= 7 && std::string(argv[6]) == "DYN";
